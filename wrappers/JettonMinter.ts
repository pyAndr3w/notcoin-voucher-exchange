...
export class JettonMinter implements Contract {
    ...
    static mintMessage(to: Address, jetton_amount: bigint, from?: Address | null, response?: Address | null, forwardPayload?: Cell | Slice | null, forward_ton_amount: bigint = 0n, total_ton_amount: bigint = 0n) {
		...
		const mintMsg = beginCell().storeUint(Op.internal_transfer, 32)
                                   ...
                                   .storeBit(byRef);
        if(byRef) {
            mintMsg.storeRef(forwardPayload);
        }
        else if(forwardPayload) {
            mintMsg.storeSlice(forwardPayload);
        }
    ...
    async sendMint(provider: ContractProvider,
				   ...
				   forward_ton_amount: bigint = toNano('0.05'), total_ton_amount: bigint = forward_ton_amount + toNano('0.05')) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: JettonMinter.mintMessage(to, jetton_amount, from, response_addr, forwardPayload, forward_ton_amount, total_ton_amount),
