...
export function computedGeneric<T extends Transaction>(transaction: T) {
    if(transaction.description.type !== "generic")
        throw new Error("Expected generic transactionaction");
...
export function storageGeneric<T extends Transaction>(transaction: T) {
    ...
    if(storagePhase  === null || storagePhase === undefined)
        throw new Error("Storage phase expected")
    return storagePhase;
}
export function getFwdStats<T extends Transaction>(transaction: T) {
    if(transaction.description.type !== "generic")
        throw new Error("Expected generic transaction");
    if(transaction.description.actionPhase == undefined)
        throw new Error("Action phase expected");
    const actionMsgSize = transaction.description.actionPhase.totalMessageSize;
    return new StorageStats(actionMsgSize.bits, actionMsgSize.cells);
}
...
export function computeMessageForwardFees(msgPrices: MsgPrices, msg: Message)  {
    ...
    if(msg.info.forwardFee == defaultFwd) {
        return {fees: { total: msgPrices.lumpPrice, res : msgPrices.lumpPrice - defaultFwd, remaining: defaultFwd}, stats: storageStats};
    }
