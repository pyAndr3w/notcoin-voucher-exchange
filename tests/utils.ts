...
import { Blockchain, SmartContract, createShardAccount } from "@ton/sandbox";
...
export const getContractData = async (address: Address, bc: Blockchain, contract?: SmartContract) => {
  const smc = contract ?? await bc.getContract(address);
...
export const setContractData = async (address: Address, data: Cell, bc: Blockchain, contract?: SmartContract) => {
    const smc = contract ?? await bc.getContract(address);
    if(!smc.account.account)
      throw("Account not found")
    if(smc.account.account.storage.state.type != "active" )
